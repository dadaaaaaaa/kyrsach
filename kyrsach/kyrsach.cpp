...
int input()
{
    ...
    for (int i = 0; i < num_finit_elements; i++) {
        finit[i] = new int[6]; // Теперь 6 узлов на треугольник
        for (int j = 0; j < 6; j++)
            _file1 >> finit[i][j];
    }
    _file1.close();
    ...
    for (int i = 0; i < num_kraev; i++) {
        kraev[i] = new int[5]; // Теперь 5 значений на каждый край
        for (int j = 0; j < 5; j++)
            _file3 >> kraev[i][j];
    }
    _file3.close();
...
void M_matrix(double* p1, double* p2, double* p3, double** M_matr, double* local_F, int num_of_area) {
    ...
    double mnoz2 = (det / 24) * gamma_betta[num_of_area][0];
...
void G_matrix(double* p1, double* p2, double* p3, double** G_matr, double gamma, int k) {
    ...
    a31 = (p1[1] - p2[1]) / det;
    a32 = (p2[0] - p1[0]) / det;
    G_matr[0][0] = a11 * a11 + a12 * a12;
    G_matr[0][1] = -2 / 3 * (a21 * a12 + a11 * a21);
        G_matr[0][2] = -2 / 3 * (a11*a31 + a12 * a32);
        G_matr[0][3] = 2 / 3 * (a21 * a11 + a22 * a12);
        G_matr[0][4] = 0;
        G_matr[0][5] = 2 / 3 * (a31 * a11 + a32 * a12);

        G_matr[1][0] = -2 / 3 * (a21 * a12 + a11 * a21);
        G_matr[1][1] = a22 * a22 + a21 * a21;
        G_matr[1][2] = -2 / 3 * (a22 * a32 + a21 * a31);
        G_matr[1][3] = 2 /3*(a12 * a22 + a11 * a21);
        G_matr[1][4] = 2 / 3 * (a32 * a22 + a31 * a21);
        G_matr[1][5] = 0;

        G_matr[2][0] = -2 / 3 * (a11 * a31 + a12 * a32);
        G_matr[2][1] = -2 / 3 * (a22 * a32 + a21 * a31);
        G_matr[2][2] = a31 * a31 + a32 * a32;
        G_matr[2][3] = 0;
        G_matr[2][4] = 2/3*(a21*a31+a22*a32);
        G_matr[2][5] = 2/3*(a11*a31+a12*a32);

        G_matr[3][0] = 2 / 3 * (a21 * a11 + a22 * a12);
        G_matr[3][1] = 2 / 3 * (a12 * a22 + a11 * a21);
        G_matr[3][2] = 0;
        G_matr[3][3] = 16/12*(a11*a11+a12*a12)+32/24*(a22*a12+a11*a21)+16/12*(a22*a22+a21*a21);
        G_matr[3][4] = 16/24*(a22*a22+a21*a21)+16/12*(a11*a31+a12*a32)+16/24*(a22*a12+a11*a21)+16/24*(a22*a32+a21*a31);
        G_matr[3][5] = 16 / 24 * (a11 * a11 + a12 * a12) + 16 / 12 * (a22 * a32 + a21 * a31) + 16 / 24 * (a22 * a12 + a11 * a21) + 16 / 24 * (a11 * a31 + a12 * a32);

        G_matr[4][0] = 0;
        G_matr[4][1] = 2 / 3 * (a32 * a22 + a31 * a21);
        G_matr[4][2] = 2 / 3 * (a21 * a31 + a22 * a32);;
        G_matr[4][3] = 16 / 24 * (a22 * a22 + a21 * a21) + 16 / 12 * (a11 * a31 + a12 * a32) + 16 / 24 * (a22 * a12 + a11 * a21) + 16 / 24 * (a22 * a32 + a21 * a31);
        G_matr[4][4] = 16/12*(a31*a31+a32*a32)+32/24*(a22*a32+a21*a31)+16/12*(a22*a22+a21*a21);
        G_matr[4][5] = 16/24*(a31*a31+a32*a32)+16/12*(a21*a12+a11*a21)+16/24*(a22*a32+a21*a31)+16/24*(a11*a31+a12*a32);

        G_matr[5][0] = 2 / 3 * (a31 * a11 + a32 * a12);
        G_matr[5][1] = 0;
        G_matr[5][2] = 2 / 3 * (a11 * a31 + a12 * a32);;
        G_matr[5][3] = 16 / 24 * (a11 * a11 + a12 * a12) + 16 / 12 * (a22 * a32 + a21 * a31) + 16 / 24 * (a22 * a12 + a11 * a21) + 16 / 24 * (a11 * a31 + a12 * a32);
        G_matr[5][4] = 16 / 24 * (a31 * a31 + a32 * a32) + 16 / 12 * (a21 * a12 + a11 * a21) + 16 / 24 * (a22 * a32 + a21 * a31) + 16 / 24 * (a11 * a31 + a12 * a32);
        G_matr[5][5] = 16/12*(a11*a11+a12*a12)+32/24*(a11*a31+a12*a32)+16/12*(a31*a31+a32*a32);
